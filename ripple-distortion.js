// ---- Cultr: RippleDistortion hero background ----
// Vanilla-JS port of the React Bits <RippleDistortion /> component.
// This site is plain HTML/JS (no React, no bundler), so the effect is
// ported to run directly against a DOM container using the same ogl
// WebGL calls the original component used — same shaders, same visual
// result, just without the React lifecycle wrapper.
//
// Usage: mountRippleDistortion(containerEl, { ...props })

import { Renderer, Program, Mesh, Geometry, Triangle, Texture, RenderTarget } from "https://esm.sh/ogl@1.0.6";
...
export function mountRippleDistortion(mount, opts = {}) {
  const {
  ...
  const setNewWave = (x, y, power) => {
    ...
    wave.target = START_SCALE * Math.max(1, spread) * power;
    wave.size = Math.max(1, brushSize);
    wave.opacity = 1;
  };
  ...
  const onMove = (event) => {
    if (!enabled || reduceMotion || trigger === "click") return;
  ...
  const onDown = (event) => {
    if (!enabled || reduceMotion || trigger === "hover") return;
    const point = localPoint(event.clientX, event.clientY);
    if (!point) return;
    setNewWave(point[0], point[1], Math.max(1, clickStrength));
  };
  ...
  const loop = (now) => {
    ...
    const decay = reduceMotion ? 1 : Math.exp((-delta * LIFE_CONSTANT) / Math.max(0.15, fade));
