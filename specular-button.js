// ---- Cultr: SpecularButton rim-shine effect ----
// Vanilla-JS port of the React Bits <SpecularButton /> component.
// This site is plain HTML/JS (no React), so instead of rendering a new
// <button>, this attaches the same ogl shader effect as a decorative
// canvas layer *inside* an existing element (an <a> or <button> you
// already have). The element keeps its normal href/onclick behavior —
// the canvas sits on top with pointer-events:none, purely visual.
//
// Usage:
//   import { mountSpecularButton } from './specular-button.js';
//   mountSpecularButton(document.querySelector('.sub-link'), { ...props });
...
export function mountSpecularButton(el, opts = {}) {
  const {
    radius = 18,
    lineColor = "#ffffff",
    baseColor = "#525252",
  ...
  // The effect needs a positioned ancestor to lay the canvas over.
  ...
  return function destroy() {
    ...
    if (fx.parentNode === el) el.removeChild(fx);
